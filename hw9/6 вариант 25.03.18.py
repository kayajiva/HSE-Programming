...
#причастия действ.прош. во всех падежных формах
    words2 = re.findall('загрузивш?[аеиу]?[а-я]?[а-я]?с?[яь]?', text)
#1 лицо ед. число буд. время
    words3 = re.findall('загружус?[яь]?', text)
#2 лицо кд. число пов.накл. 
    words4 = re.findall('(загрузи)[ .,!?:;]', text)
#всё остальное
    words5 = re.findall('загруз[ия][мтшл][иаоеь]?с?[яь]?', text)
